...
// v45: Configurable occupancy target
extern "C" __launch_bounds__(BLOCK_SIZE, V45_OCCUPANCY)
...
__global__ void KernelB(const TKparams Kparams)
{
	...
	{	
		...
		{
			// v50 FIX: Was BLOCK_X (blockIdx.x), must be THREAD_X (threadIdx.x)
			// Bug: all 256 threads per block shared same loop detection entry
		...
		for (int i = 0; i < MD_LEN; i++)
		{
			...
			// v50 FIX: Was BLOCK_X, must be THREAD_X
...
__global__ void KernelC(const TKparams Kparams)
{
	...
	while (1)
	{
		...
		// v38.2 FIX: Use 32-bit atomicAnd (3-4x faster than 64-bit)
		// Split into two 32-bit operations based on gr_ind
		{
			u32* L1S2_as_u32 = (u32*)&Kparams.L1S2[block_ind * BLOCK_SIZE + thr_ind];
			if (gr_ind < 32) {
				atomicAnd(&L1S2_as_u32[0], ~(1u << gr_ind));
			} else {
				atomicAnd(&L1S2_as_u32[1], ~(1u << (gr_ind - 32)));
			}
		}
	}
}
...
__global__ void KernelGen(const TKparams Kparams)
{
	for (u32 group = 0; group < PNT_GROUP_CNT; group++)
	{
		...
		{
			// AllWildsMode: ALL kangaroos get PubKey offset (100% WILDs)
			// Normal mode: only kang_ind >= KangCnt/3 get offset
			if (Kparams.AllWildsMode || kang_ind >= Kparams.KangCnt / 3)
			{
