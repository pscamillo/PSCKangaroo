...
// =====================================================================
// ENDOMORPHISM 6x FUNCTIONS
// β³ ≡ 1 (mod p), used to compute equivalent X coordinates
// =====================================================================

// BETA constants for secp256k1 endomorphism
#define BETA_0  0xC1396C28719501EEull
#define BETA_1  0x9CF0497512F58995ull  
#define BETA_2  0x6E64479EAC3434E9ull
#define BETA_3  0x7AE96A2B657C0710ull

// v55 FIX: BETA2 was wrong! Old values were NOT a cube root of unity.
// Correct: beta^2 mod p = 0x851695D49A83F8EF 919BB86153CBCB16 630FB68AED0A766A 3EC693D68E6AFA40
// Verified: beta^3 mod p = 1 (cube root of unity on secp256k1)
...
// =====================================================================
// BRANCHLESS COMPARISON AND SELECTION (Gemini optimization)
// Avoids warp divergence by using predicated selection
// =====================================================================

// Compare two 256-bit numbers: returns 1 if a < b, 0 otherwise (branchless)
__device__ __forceinline__ u32 IsLess256(u64* a, u64* b)
{
    // Compare from high to low limb
    // Result: 1 if a < b, 0 otherwise
    
    // Check if high limbs determine order
    u32 lt3 = (a[3] < b[3]) ? 1 : 0;
    u32 gt3 = (a[3] > b[3]) ? 1 : 0;
    u32 eq3 = (a[3] == b[3]) ? 1 : 0;
    ...
    // Combine: a < b if (a[3] < b[3]) OR (a[3]==b[3] AND a[2]<b[2]) OR ...
    u32 result = lt3 | (eq3 & (lt2 | (eq2 & (lt1 | (eq1 & lt0)))));
    return result;
}

// Conditional select: dst = sel ? src_true : src_false (branchless using PTX selp)
__device__ __forceinline__ void Select256(u64* dst, u64* src_true, u64* src_false, u32 sel)
{
    // Use ternary operator - compiler should optimize to selp instruction
    dst[0] = sel ? src_true[0] : src_false[0];
    dst[1] = sel ? src_true[1] : src_false[1];
    dst[2] = sel ? src_true[2] : src_false[2];
    dst[3] = sel ? src_true[3] : src_false[3];
}

// Multiply X by BETA mod p (constant propagation optimized)
__device__ __forceinline__ void MulByBeta(u64* res, u64* x)
{
    // BETA is a compile-time constant - helps with optimization
    __align__(16) u64 beta[4] = { BETA_0, BETA_1, BETA_2, BETA_3 };
    MulModP(res, x, beta);
}

// Multiply X by BETA^2 mod p (constant propagation optimized)
__device__ __forceinline__ void MulByBeta2(u64* res, u64* x)
{
    __align__(16) u64 beta2[4] = { BETA2_0, BETA2_1, BETA2_2, BETA2_3 };
    MulModP(res, x, beta2);
}

// Get canonical X (smallest of X, β·X, β²·X) and return transform used
// Returns: 0 = original, 1 = beta, 2 = beta²
// BRANCHLESS VERSION - avoids warp divergence (Gemini optimization)
__device__ __forceinline__ u32 GetCanonicalX(u64* x_out, u64* x_in)
{
    __align__(16) u64 x1[4], x2[4], temp[4];
    
    // Compute β·X and β²·X
    MulByBeta(x1, x_in);
    MulByBeta2(x2, x_in);
    
    // Branchless minimum selection
    // Step 1: Compare x1 vs x_in, select smaller into temp
    u32 sel1 = IsLess256(x1, x_in);
    Select256(temp, x1, x_in, sel1);
    u32 transform = sel1;  // 1 if x1 was selected, 0 if x_in
    
    // Step 2: Compare x2 vs temp, select smaller into x_out
    u32 sel2 = IsLess256(x2, temp);
    Select256(x_out, x2, temp, sel2);
    
    // Update transform: if sel2, use 2; else keep previous
    // This is: transform = sel2 ? 2 : transform
    transform = sel2 * 2 + (1 - sel2) * transform;
